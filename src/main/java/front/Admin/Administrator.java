...
public class Administrator {
	...
	private void addItem() {
		...
		distributore.getManager().getItems().add(new Item(codice, nome, prezzo, qty));
		distributore.getManager().saveItems();
	}
	...
	private void delItem() {
		...
		for (Item e : distributore.getManager().getItems()) {
			if(e.getCodice() == codice) {
				distributore.getManager().getItems().remove(e);
	...
	private void modCode() {
		...
		distributore.getManager().getItem(i -> i.getCodice() == codice).setCodice(newCodice);
		distributore.getManager().saveItems();
...
//		distributore.getItems().getItems().get(getIndex(codice)).setCodice(newCodice);
	...
	private void modNome() {
		...
		distributore.getManager().getItem(i -> i.getCodice() == codice).setNome(nome);
		distributore.getManager().saveItems();
	...
	private void modQty() {
		...
		distributore.getManager().getItem(i -> i.getCodice() == codice).setQty(qty);
		distributore.getManager().saveItems();
	...
	private int getMessageCode() {
		...
		if(codice < 0) {
			...
			codice = distributore.getManager().getItems().size() + 1;
